Describe "Validating the module manifest" {
	...
	$manifest = ((Get-Content "$moduleRoot\PSAADApplication\PSAADApplication.psd1") -join "`n") | Invoke-Expression
	<#
	Context "Basic resources validation" {
		$files = Get-ChildItem "$moduleRoot\src\public" -Recurse -File | Where-Object Name -like "*.ps1"
		...
		It "Exports none of its internal functions" -TestCases @{ moduleRoot = $moduleRoot; manifest = $manifest } {
			$files = Get-ChildItem "$moduleRoot\src\private" -Recurse -File -Filter "*.ps1"
